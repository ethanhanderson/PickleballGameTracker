...
/// Parses URLs into typed deep link destinations.
public struct DeepLinkResolver: Sendable {
  /// Create a resolver.
  public init() {}

  /// Resolve a URL into a `DeepLinkDestination`.
  /// - Parameter url: The incoming URL (custom scheme or universal link).
  /// - Throws: `DeepLinkError` when the URL cannot be parsed or is unsupported.
  /// - Returns: A typed destination for in-app navigation.
  public func resolve(_ url: URL) throws -> DeepLinkDestination {
    ...
    guard let entityRaw = entityOpt, !entityRaw.isEmpty else {
