...
import SwiftUI
  ...
  public final class HapticFeedbackService: ObservableObject, Sendable {
    ...
    // Centralized enablement gate
    private func performIfEnabled(_ work: () -> Void) {
      if isEnabled { work() }
    }

    /// Trigger haptic feedback for scoring actions
    public func scoreAction() { performIfEnabled { impactMedium.impactOccurred() } }

    /// Trigger haptic feedback for serve changes
    public func serveChange() { performIfEnabled { impactLight.impactOccurred() } }

    /// Trigger haptic feedback for game state changes (timeout, rally, etc.)
    public func gameStateChange() { performIfEnabled { impactLight.impactOccurred() } }

    /// Trigger haptic feedback for timer start/stop
    public func timerToggle() { performIfEnabled { impactMedium.impactOccurred() } }

    /// Trigger haptic feedback for game completion
    public func gameComplete() { performIfEnabled { notificationGenerator.notificationOccurred(.success) } }

    /// Trigger haptic feedback for new game start
    public func gameStart() { performIfEnabled { impactHeavy.impactOccurred() } }

    /// Trigger haptic feedback for button selections
    public func buttonSelection() { performIfEnabled { selectionGenerator.selectionChanged() } }

    /// Trigger haptic feedback for errors
    public func error() { performIfEnabled { notificationGenerator.notificationOccurred(.error) } }

    /// Trigger haptic feedback for warnings
    public func warning() { performIfEnabled { notificationGenerator.notificationOccurred(.warning) } }
