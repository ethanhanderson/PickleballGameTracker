...
public final class PlayerTeamManager {
  ...
  public func refreshPlayers() {
    players = (try? storage.loadPlayers(includeArchived: false)) ?? []
  }

  public func refreshTeams() {
    teams = (try? storage.loadTeams(includeArchived: false)) ?? []
  }

  public func refreshPresets() {
    presets = (try? storage.loadPresets(includeArchived: false)) ?? []
  }

  // MARK: - Archive Loaders
  public func loadArchivedPlayers() -> [PlayerProfile] {
    (try? storage.loadPlayers(includeArchived: true).filter { $0.isArchived }) ?? []
  }

  public func loadArchivedTeams() -> [TeamProfile] {
    (try? storage.loadTeams(includeArchived: true).filter { $0.isArchived }) ?? []
  }
