...
public final class ActiveGameStateManager: Sendable {
  ...
  /// - Parameters:
  ///   - modelContext: SwiftData model context used for queries and saves.
  ///   - gameManager: Optional game manager to delegate operations to.
  ///   - enableSync: Whether to enable cross-device synchronization.
  ...
  /// - Throws: `GameError.noActiveGame` when no current game is set.
  ...
  /// - Throws: `GameError.noActiveGame` if no game is set.
  ...
  /// Pause the game and automatically stop the timer.
  /// - Throws: `GameError.noActiveGame` if no game is set.
  ...
  /// Resume the game and automatically resume the timer.
  /// - Throws: `GameError.noActiveGame` if no game is set.
  ...
  private func startTimerInternal() {
    ...
    // Ensure timer fires during UI interactions/scrolling
    if let gameTimer {
