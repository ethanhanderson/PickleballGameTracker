...
import SwiftUI
  ...
  public actor ActiveGameSyncService: ObservableObject {
    ...
    /// Send active game state to paired device
    ...
    /// Request game history from paired device
    ...
    /// Send game history batch to paired device
    ...
    private func sendMessage(_ message: SyncMessage) async throws {
      ...
      if session.isReachable {
        ...
        try session.transferUserInfo(["message": data])
      }
    }
    ...
    nonisolated func updateLastError(_ error: Error?) {
      Task { @MainActor in
        lastError = error
      }
    }
